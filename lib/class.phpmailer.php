...
class phpmailer
{
    ...
    function SendToQueue($queue_path, $send_time = 0) {
        ...
        {
            $message[] = sprintf("Sender: %s%s", $this->Sender, $this->LE);
            $message[] = sprintf("Subject: =?UTF-8?B?%s?= %s", base64_encode(trim($this->Subject)), $this->LE);
    ...
    function create_header() {
        ...
        $header[] = sprintf("From: =?UTF-8?B?%s?= <%s>%s", base64_encode(addslashes($this->FromName)), 
        ...
        if(count($this->ReplyTo) > 0)
        	$header[] = sprintf("Reply-To: %s%s",trim($this->ReplyTo), $this->LE);
           // $header[] = $this->addr_append("Reply-To: ", $this->ReplyTo);

        // mail() sets the subject itself
        if($this->Mailer != "mail")
            $header[] = sprintf("Subject: =?UTF-8?B?%s?= %s", base64_encode(trim($this->Subject)), $this->LE);
